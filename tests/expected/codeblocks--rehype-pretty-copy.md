...
Some intro text here to make the article long enough for domdown to pick it up as content. This is a test article with enough words to pass the word count threshold. We need to make sure domdown picks this up properly so we can test the code block behavior with rehype-pretty-copy output.
