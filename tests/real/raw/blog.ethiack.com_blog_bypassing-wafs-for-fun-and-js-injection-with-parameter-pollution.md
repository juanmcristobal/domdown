...
author:
  - "Bruno Mendes"
published: "2025-08-04 08:19:26"
description: "Technical deep dive into bypassing a strict Web Application Firewall using HTTP Parameter Pollution, leveraging multi-parameter payload splitting to achieve JavaScript injection and evade detection."
---
# Bypassing WAFs for Fun and JS Injection with Parameter Pollution

[![](https://blog.ethiack.com/hs-fs/hubfs/1657583561080.jpeg?width=150&height=150&name=1657583561080.jpeg)](https://blog.ethiack.com/blog/author/bruno-mendes)
...
| **Golang net/http - r.URL.Query()[“param”]** | param=val1¶m=val2 | param=['val1','val2'] |
...
Machine Learning-Based WAFsrepresent the next step in web application security. These systems analyze traffic patterns and user behavior to identify anomalous requests that may indicate attacks. Unlike signature-based systems, ML-powered WAFs can potentially detect 0-days and previously unknown attack vectors. This obviously comes at a cost as a low false positive rate may not be guaranteed and if not combined with a good pre-trained source of truth, may leave applications with low traffic levels susceptible to attacks.
...
| **open-appsec** | Medium+ Sensitivity | Default, Basic Model |
| **open-appsec** | High+ Sensitivity | Default, Basic Model |
| **open-appsec** | Only Critical Sensitivity | Default, Basic Model |
...
*Results obtained using the Basic Model in the default configuration of open-appsec.
...
test\\';alert(1);//This payload leverages a subtle parsing discrepancy in how Azure WAF handles escaped characters compared to how Javascript processes them. The double backslash and single quote combination (\\’) is interpreted differently by the WAF's pattern matching and the Javascript parser. While Javascript will escape the backslack that is escaping the quote, Azure WAF will interpret the backslack escaping the quote as an escaped quote.

![image (21)](https://blog.ethiack.com/hs-fs/hubfs/image%20(21).png?width=1503&height=1689&name=image%20(21).png)
...
![image (23)](https://blog.ethiack.com/hs-fs/hubfs/image%20(23).png?width=2616&height=5652&name=image%20(23).png)
...
![image (25)](https://blog.ethiack.com/hs-fs/hubfs/image%20(25).png?width=2625&height=1479&name=image%20(25).png)

Since open-appsec relies on machine learning to make its decisions, we decided to see if our hackbot could adapt if the original payload got blocked. To do this, we generated enough traffic so that the _alert_based payload started being blocked by the WAF, and we let the hackbot do its magic.

This time, almost instantly, it found a bypass using _confirm_instead of _alert_.

![image (26)-1](https://blog.ethiack.com/hs-fs/hubfs/image%20(26)-1.png?width=2640&height=1284&name=image%20(26)-1.png)
