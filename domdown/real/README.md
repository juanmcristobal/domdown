...
- `manifest.json` declares the cases, their thresholds, and any intentional xfails.

## Adding a case

1. Copy the source HTML into `html/<case-id>.html`.
2. Copy the expected Markdown into `raw/<case-id>.md`.
3. Add an entry to `manifest.json` with the relative paths and a `min_ratio`.
